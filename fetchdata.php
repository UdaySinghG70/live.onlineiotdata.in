...
//$_REQUEST['device_id']="DAPO_TAME";

if(isset($_REQUEST['username']) && isset($_REQUEST['password']) && isset($_REQUEST['device_id'])){
	...
	$current_data = $ddao->getCurrentData($device_id );
	...
	if($deviceParams!=null && $current_data!=null){
		...
		for ($j=0; $j<count($row); $j++){
			...
			if($deviceParams[$j]->param_type=="datetime"){
				...
				$date_time=$value;
			}else if(strtolower(str_replace(' ', '', $deviceParams[$j]->param_name))=="waterlevel"){
				$water_level = $value;
			}else if(strtolower(str_replace(' ', '', $deviceParams[$j]->param_name))=="batteryvoltage"){
