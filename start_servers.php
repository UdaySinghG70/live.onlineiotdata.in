...
$websocket_cmd = 'nohup php -d display_errors=1 -d error_reporting=E_ALL websocket_server.php >> websocket.log 2>&1 &';
exec($websocket_cmd);
echo "WebSocket server started (check websocket.log for output)\n";

// Start MQTT client in background with nohup
$mqtt_cmd = 'nohup php -d display_errors=1 -d error_reporting=E_ALL mqtt_client.php >> mqtt.log 2>&1 &';
exec($mqtt_cmd);
